...
export default function PaymentDetails({
  ...
  return (
    <div className="space-y-4">
      {/* Reference number */}
      <div className="bg-[#1A1A4E]/5 border border-[#1A1A4E]/10 rounded-xl px-4 py-3 flex items-center justify-between">
        <div>
          ...
          {referenceNumber ? (
            ...
            <p className="text-xs text-gray-400 font-medium mt-0.5">Reference pending — check your WhatsApp confirmation</p>
          )}
        </div>
        {referenceNumber && (
          <button onClick={() => copyToClipboard(referenceNumber, 'ref')} className="flex items-center gap-1 text-xs font-black text-pink-400 hover:text-pink-600 transition-colors">
            {copied === 'ref' ? <Check size={13} className="text-green-500" /> : <Copy size={13} />}
      ...
      >
        ...
        Paid? Share Payment Screenshot on WhatsApp
      </a>
      <p className="text-[11px] text-center text-gray-400 -mt-2">
        After paying, tap above and attach your payment screenshot in WhatsApp — or our team will contact you directly to confirm.
      </p>
      ...
      {onDone && (
        <button onClick={onDone} className="w-full bg-[#1A1A4E] hover:bg-[#2D1B6B] text-white font-black py-3.5 rounded-xl transition-colors text-sm">
          Done
        </button>
      )}
    </div>
